...
def unfold_correlation(input1, input2, K=1, dilation=1):
      ...
      temp = F.unfold(input2, kernel_size=3, padding=1)
      temp = temp.view(batch,c,3,3,h,w)
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(args.times)):
        scs_out = scs_correlation(input1, input2, P=args.patch)
    end = time.time()
    print("scs", (end-start) / args.times)

    start = time.time()
    for i in tqdm(range(args.times)):
        unfold_out = unfold_correlation(input1, input2, K=args.patch)
    end = time.time()
