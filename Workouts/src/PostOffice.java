...
class PostOffice {
	...
	static HashMap<Point, Integer> Memoizer = null;
	...
	public static void main(String[] args) {
		...
		Memoizer = new HashMap<Point, Integer>();
	...
	private static int getNumberOfPaths(int i_x, int j_y,
		...
		if(pathLength > nMinCost) return 0;
		
		// exit criterias
		// if the given point is invalid, return 0
		if (!isValidPoint(i_x, j_y))
			return 0;
		...
		// if the given point is a construction point, return 0
		if (isConstructionPoint(i_x, j_y))
			return 0;
		...
		int right = getNumberOfPaths(i_x, j_y + 1, cloneList(seenPoints));
		int down = getNumberOfPaths(i_x + 1, j_y, cloneList(seenPoints));
		int left = getNumberOfPaths(i_x, j_y - 1, cloneList(seenPoints));
		int up = getNumberOfPaths(i_x - 1, j_y, cloneList(seenPoints));
	...
	private static boolean hasSeen(int i_x, int j_y, ArrayList<Point> seenPoints) {
		...
		// of the seen points
		for (Iterator<Point> iterator = seenPoints.iterator(); iterator
				.hasNext();) {
	...
	private static boolean isConstructionPoint(int i_x, int j_y) {
		for (Iterator<Point> iterator = constructionPoints.iterator(); iterator
				.hasNext();) {
			Point point = (Point) iterator.next();
			if (point.isEqual(i_x, j_y)) {
				return true;
