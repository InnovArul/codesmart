function [ maxlikelihood, classLabels, classLabelsProbability ] = getBayesianMaxLikelihood( model, priors, testData )
...
for classNumber = 1:k
  ...
  mahalanobis = sum(0.5 * (meandiff * inv(unSingularify(model{classNumber, SIGMA_INDEX}))) .* meandiff, 2);
  classLabelsProbability(:, classNumber) = log(exp(-mahalanobis)  / (((2 * pi)^(n/2)) * sqrt(det(unSingularify(model{classNumber, SIGMA_INDEX})))));
end
