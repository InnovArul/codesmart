...
switch(caseNumber)
    case 1
        ...
        covMatrices = reshape([model{:, 2}], n, n, size(model, 1));
        [model{:, 2}] = deal(mean(covMatrices, 3) + eps); %mean(covMatrices, 3));
    ...
    case 3
        ...
        [model{:, 2}] = deal(maxSigma2 * eye(size(meanCov)) + eps); %mean(covMatrices, 3)); 
    ...
    case 4
        ...
        [model{:, 2}] = deal(meanCov + eps); %mean(covMatrices, 3));          
    case 5
        % 5 -- Naive Bayes with C different for all
        ...
        for classNumber = 1 : size(model, 1)
            ...
            model{classNumber, 2} = covariance  + eps;
        end
    otherwise
