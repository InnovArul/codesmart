function [ROCBuffer] = plotDETcurve(actualLabels, classLabelsProbability, noOfClasses)
...
for classNumber = 1 : noOfClasses
    ...
    for thresIdx = 1:(size(sortedProbability))
        ...
        X(thresIdx + 1) = log(fpCount/(totalFalse));
        Y(thresIdx + 1) = log(1 - (tpCount/(totalTrue)));
    end
