...
train_file = 'svhn.t7/train_32x32.t7'
...
testData = {
   data = loaded.data,
   -- subtract one from the labels of MNIST
   labels = loaded.labels - 1,
   size = function() return tesize end
}

-- make sure that the label of picture 0 is assigned 10 , as per the model from SVHN netwrok
testData.labels:apply(function(x) if x==0 then return 10 else return x end end)
----------------------------------------------------------------------
...
print '==> preprocessing data: colorspace RGB -> Y'
trDataSize = trainData.data:size()

--create a new tensor to hold the gray images of SVHN dataset
newData = torch.Tensor(trDataSize[1], 1, trDataSize[3], trDataSize[4])

--convert SVHN data to y channel (a single channel)
for i = 1,trainData:size() do
   newData[i] = image.rgb2y(trainData.data[i])
end

trainData.data = newData:float()

-- Name channel for convenience (the same code as of 3 channel SVHN training)
channels = {'g'}
...
-- using these values.
print '==> preprocessing data: normalize feature (channel) globally'
mean = {}
std = {}
...
-- Local normalization
print '==> preprocessing data: normalize channel locally'
...
if opt.visualize then
   ...
   first256Samples_y_test = testData.data[{ {1,256},1 }]

   if itorch then      
      itorch.image(first256Samples_y)
      itorch.image(first256Samples_y_test)
   else
      image.display(first256Samples_y)
      image.display(first256Samples_y_test)
   end
end
